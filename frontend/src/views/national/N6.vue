  ...
  <div>
    ...
    <v-card :loading="n6.loading">
      <v-card-text>
        ...
        ></v-skeleton-loader>
        <v-simple-table v-if="!n6.loading" class="sticky" dense fixed-header>
          <template v-slot:default>
            <thead>
              <tr class="text-uppercase">
                ...
                <th
                  ...
                  colspan="2"
                  v-for="(annee, indexAnnee) in n6.annee"
                  :key="indexAnnee"
                >
                  {{ annee }}
                </th>
              ...
              <tr class="text-uppercase">
                <template v-for="(annee, indexAnnee) in n6.annee">
                  <th
                    class="white--text text-center primary border-left"
                    :key="indexAnnee + '1'"
                  >
            ...
            <tbody>
              <tr
                v-for="(departement, indexDepartement) in n6.departement"
...
export default {
  ...
  computed: {
    alert: {
      get () {
        return this.$store.state.national.n6.alert
      },
      set (newValue) {
    ...
    filter: {
      get () {
        return this.$store.state.national.n6.filter
      },
      set (newValue) {
        return this.$store.dispatch('national/property', {
          n: 'n6',
    ...
    n6: {
      get () {
        return {
          loading: this.$store.state.national.n6.loading,
          departement: this.$store.getters['national/n6Departement'],
          annee: this.$store.state.national.n6.annee
        }
      }
    }
  },
  created () {
    this.$store.dispatch('national/n6')
  },
  mounted () {
